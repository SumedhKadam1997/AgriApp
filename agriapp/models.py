...
class Crops(db.Model):
    ...
    foliar = db.relationship('Foliar', backref = "maincrop", lazy = True)
...
class Foliar(db.Model):
    __tablename__ = "foliarspray"
    crops = db.relationship(Crops)
    ...
    content3 = db.Column(db.Integer)
    content4 = db.Column(db.Integer)
    cost = db.Column(db.Integer)
    notes = db.Column(db.Text)

    def __init__(self, crop_id, date_of_apply, content1, content2, content3, content4, cost, notes):
        self.crop_id = crop_id
        self.date_of_apply = date_of_apply
...
admin.add_view(MyModelView(Foliar, db.session))
